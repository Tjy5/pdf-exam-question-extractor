# PaddleOCR 完整使用指南（基于 3.x，更新至 2024-10 官方文档）

> **📢 v2.0.1 架构更新**: 本项目已于2025-12-12完成路径解析增强和数据库持久化。OCR相关代码已模块化到 `src/common/ocr_models.py`。如需了解项目架构变更，请先阅读 [项目说明 & 给后续 AI 的提示.md](项目说明%20&%20给后续%20AI%20的提示.md)。
...
在v2.0架构中，OCR相关代码已重构为模块化结构：

```python
# v2.0推荐导入方式
from src.common.ocr_models import (
...
# 使用示例
pipeline = get_ppstructure()  # 获取单例，避免重复初始化
doc = pipeline.predict("page.png")[0]
blocks = layout_blocks_from_doc(doc)  # 标准化版面块
```
...
- 主要组件对应的安装 extra：
  - 基础 OCR：`paddleocr`
  - 文档解析 / 结构化 / VL：`paddleocr[doc-parser]`
  - 信息抽取（ChatOCR）：`paddleocr[ie]`
  - 文档翻译（DocTranslation）：`paddleocr[trans]`（本项目当前未使用）
  - 本项目推荐：**至少安装 `paddleocr[doc-parser]`**，以支持 PP-StructureV3 和 PaddleOCR-VL。

> 如果你此前使用的是 PaddleOCR 2.x：3.x 在命令行和配置方式上变动较大，请优先参考官方 README 的 3.0 升级说明，不要混用旧版示例和本指南。
...
根据需求选择不同 extra（本项目场景推荐 `doc-parser` + 可选 `ie`，如需文档翻译可选 `trans`）：
...
# 本项目核心（PP-StructureV3 / PaddleOCR-VL）
python -m pip install -U "paddleocr[doc-parser]"

# 如需使用 PP-ChatOCRv4 进行信息抽取
python -m pip install -U "paddleocr[ie]"

# 如需使用 DocTranslation（文档翻译）
python -m pip install -U "paddleocr[trans]"
```

### 2.3 离线模型缓存（无网环境）

- PaddleX 官方模型默认需联网下载，可提前把模型目录放到  
  **Windows**: `%USERPROFILE%\\.paddlex\\official_models`  
  **Linux/macOS**: `~/.paddlex/official_models`
- 本项目已补丁：若检测到无可用 hoster，会自动从上述路径读取（例如 `PP-DocBlockLayout`、`PP-Chart2Table` 等），无需联网。
- 预下载方式：可在有网环境跑一遍，或手动从 Paddle 官方发布包解压到对应子目录，保持原有文件结构（包含 `inference.pdiparams` / `config.json` / `inference.yml` 等）。
...
> 说明：早期示例中常见的 `--use_mobile_model` 参数在 3.x 中已不再推荐使用，3.x 下通常通过模型配置或权重路径来区分 server/mobile 版本，具体以官方 README 中的示例为准。
...
> 本仓库中的 `extract_questions_ppstruct.py` / `make_data_analysis_big.py` / `compose_question_long_image.py` / `common.py` 等脚本，
> 都是在 PP-StructureV3 提供的 `parsing_res_list` 基础上，通过题号正则 + 分区标题关键字来完成
> “按题切图 + 跨页续接 + 资料分析大题聚合 + 长图拼接”的规则层处理，模型部分完全使用官方产线，无需重新训练。
...
- 第 1 层（按题号切单题）：`extract_questions_ppstruct.py`
  ...
  - 使用 `common.get_ppstructure()` 获取单例流水线，`layout_blocks_from_doc()` 做版面标准化；
  - `compute_smart_crop_box()` + `detect_continuation_blocks()` 负责智能裁剪和跨页续接。
- 第 2 层（资料分析大题聚合 + 长图）：`make_data_analysis_big.py` + `compose_question_long_image.py`
  - 继续使用 **PP-StructureV3** 分析资料分析段落，生成大题截图并写入 `big_questions`；
  - 利用 `common.crop_page_and_save()` 和 `compose_vertical()` 将多页材料/小题拼成长图，并写入 `combined_image` / `long_image` 字段。
- 第 3 层（大题级语义理解）：`run_chatocr_on_big_questions.py` / `run_vl_on_big_questions.py`
  - 前者调用 **PP-ChatOCRv4Doc**，基于大模型对资料分析大题截图做摘要 / 信息抽取；
  - 后者调用 **PaddleOCR-VL**，对大题截图做多模态结构化解析，输出 JSON；
  - 两者都在原有 `meta.json` 基础上**追加字段**，不会破坏已有结构。

### 7.2 公共模块 `common.py` 的角色（与 PaddleOCR 的关系）

- 把 PaddleOCR 提供的基础能力（PP-StructureV3、PaddleOCR-VL 等）包装成可复用的工具函数：
  - 避免在每个脚本里重复初始化模型或手写路径解析逻辑；
  - 把“题号正则、分区标题关键字、跨页判断策略”等规则集中管理，便于后续微调。
- 你可以把 `common.py` 理解为：在 PaddleOCR 官方产线之上的一层“试卷业务逻辑封装”。
...
- 推荐阅读顺序：
  ...
  6. 文档翻译产线 PP-DocTranslation（对应 `paddleocr[trans]`）  
  7. （如有需要）官方仓库中关于 MCP Server / 部署的章节
